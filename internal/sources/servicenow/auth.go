...
// waits for the user to log in to ServiceNow via SSO, and extracts the
// session token (g_ck) and cookies. The token is saved to configDir.
func RunAuth(instanceURL, configDir string, progress ProgressFunc) (*TokenData, error) {
	if progress == nil {
		progress = func(_, _ string) {}
	}

	// Write the embedded auth script and package.json to a stable dir in configDir.
	scriptDir := filepath.Join(configDir, "servicenow-auth")
	...
	profileDir := filepath.Join(configDir, "servicenow-browser-profile")

	progress("launching", "Opening browser...")

	// Run the auth script. Token JSON goes to stdout; progress JSON lines go to stderr.
	cmd := exec.Command("node", filepath.Join(scriptDir, "auth.js"), instanceURL, profileDir)
	cmd.Dir = scriptDir
...
// instanceName extracts a safe identifier from an instance URL.
